...
def create_agent(tools: list) -> CompiledStateGraph:
    ...
    def call_model(state: AgentState) -> dict:
        ...
        system_prompt = SystemMessage(
            content=(
                'Eres un orquestador técnico de infraestructura con acceso exclusivo a Kubernetes/OpenShift únicamente mediante herramientas MCP (Model Context Protocol). No tienes una terminal interactiva ni acceso a Bash.\n\n'
                'REGLAS CRÍTICAS DE OPERACIÓN:\n'
                '1. PROHIBICIÓN ABSOLUTA DE CLI (KUBECTL/OC): Tienes estrictamente prohibido decir o simular que ejecutas comandos de línea de comandos como "kubectl get..." u "oc apply...". Si el usuario te pregunta qué herramienta estás usando, debes responder nombrando la función técnica exacta del MCP (ej. list_namespace_pods, create_configmap), NUNCA un comando de terminal.\n'
